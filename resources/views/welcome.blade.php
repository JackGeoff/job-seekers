...
<body class="min-w-0 overflow-x-hidden bg-[#fafaff] font-sans text-slate-950 antialiased" x-data="{ mobileOpen: false, profileOpen: false, profile: 'job' }" @keydown.escape.window="mobileOpen = false; profileOpen = false" :class="{ 'overflow-hidden': profileOpen }">
    <header class="relative z-40 border-b border-indigo-100/70 bg-[#fafaff]/90 backdrop-blur">
        <nav class="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-10" aria-label="Main navigation">
            ...
            <div class="hidden items-center gap-3 sm:flex">
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-indigo-50 hover:text-indigo-700 focus-visible:outline-2 focus-visible:outline-indigo-600">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-indigo-50 hover:text-indigo-700 focus-visible:outline-2 focus-visible:outline-indigo-600">Sign In</a><a href="{{ route('register') }}" class="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-200 transition duration-200 hover:-translate-y-0.5 hover:bg-indigo-700 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">Get Started</a>
                @endauth
            </div>
        ...
        <div id="mobile-menu" x-cloak x-show="mobileOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-y-2 opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-end="-translate-y-2 opacity-0" class="border-t border-indigo-100 bg-white px-5 py-4 shadow-lg sm:hidden"><div class="mx-auto grid max-w-7xl gap-1"><a @click="mobileOpen = false" href="#" class="rounded-xl px-4 py-3.5 text-base font-semibold text-slate-700 hover:bg-indigo-50">Browse Jobs</a><a @click="mobileOpen = false" href="#" class="rounded-xl px-4 py-3.5 text-base font-semibold text-slate-700 hover:bg-indigo-50">Companies</a><a @click="mobileOpen = false" href="#" class="rounded-xl px-4 py-3.5 text-base font-semibold text-slate-700 hover:bg-indigo-50">Resources</a><div class="mt-3 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4">@auth<a href="{{ route('dashboard') }}" class="col-span-2 rounded-xl bg-indigo-600 px-4 py-3 text-center text-sm font-semibold text-white">Dashboard</a>@else<a href="{{ route('login') }}" class="rounded-xl border border-slate-200 px-4 py-3 text-center text-sm font-semibold text-slate-700">Sign In</a><a href="{{ route('register') }}" class="rounded-xl bg-indigo-600 px-4 py-3 text-center text-sm font-semibold text-white">Get Started</a>@endauth</div></div></div>
    </header>
    <main><section class="relative isolate overflow-hidden"><div class="pointer-events-none absolute left-1/2 top-0 -z-10 h-96 w-96 -translate-x-1/2 rounded-full bg-indigo-100/45 blur-3xl"></div><div class="mx-auto grid max-w-7xl gap-12 px-5 pb-14 pt-14 sm:px-8 sm:pb-20 sm:pt-20 lg:grid-cols-[1.04fr_.96fr] lg:items-center lg:gap-14 lg:px-10 lg:py-24"><div class="min-w-0">
        ...
        <dl class="rise-in-late mt-10 grid grid-cols-3 gap-2 border-t border-slate-200 pt-6 sm:max-w-lg sm:gap-6"><div><dt class="mb-2 grid size-7 place-items-center rounded-lg bg-indigo-50 text-indigo-600">◷</dt><dd class="text-xl font-extrabold tracking-tight">500+</dd><dd class="mt-0.5 text-xs font-medium text-slate-500">Active Jobs</dd></div><div><dt class="mb-2 grid size-7 place-items-center rounded-lg bg-violet-50 text-violet-600">⌂</dt><dd class="text-xl font-extrabold tracking-tight">2K+</dd><dd class="mt-0.5 text-xs font-medium text-slate-500">Companies</dd></div><div><dt class="mb-2 grid size-7 place-items-center rounded-lg bg-fuchsia-50 text-fuchsia-600">♙</dt><dd class="text-xl font-extrabold tracking-tight">10K+</dd><dd class="mt-0.5 text-xs font-medium text-slate-500">Job Seekers</dd></div></dl>
...
<section class="border-t border-indigo-100/70 bg-white py-14 sm:py-20">
    <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        ...
        @if ($jobs->isNotEmpty())
            <div class="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($jobs as $job)
                    <article class="flex h-full flex-col rounded-2xl border border-slate-200 bg-[#fafaff] p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                        <div class="flex-1">
                            <p class="text-xs font-bold uppercase tracking-[.12em] text-indigo-600">{{ $job->category }}</p>
                            <h3 class="mt-2 text-xl font-bold text-slate-950">{{ $job->title }}</h3>
                            <p class="mt-1 font-semibold text-slate-700">{{ $job->employerProfile?->company_name }}</p>
                            ...
                            @if ($job->application_deadline)
                                <p class="mt-4 text-xs font-semibold text-slate-500">Apply by {{ $job->application_deadline->format('M d, Y') }}</p>
                            @endif
                        </div>
                        <a href="{{ route('jobs.show', $job) }}" class="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-4 text-sm font-bold text-white transition hover:bg-indigo-700">View Job</a>
                    </article>
                @endforeach
            </div>
        @else
